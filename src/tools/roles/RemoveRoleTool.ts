...
export class RemoveRoleTool extends BaseTool {
    readonly name = 'remove_role';
    readonly description = 'Remove a role from a member.';
    ...
    readonly parameters = z.object({
        ...
        role: z.string().describe('The name of the role to remove'),
        reason: z.string().optional().describe('The reason for removing the role'),
    });
    ...
    async execute(context: ToolExecutionContext): Promise<ToolResult> {
        ...
        await member.roles.remove(roleObj, `[Overseer] ${reason ?? 'No reason'} (by ${context.requester.displayName})`);

        return { success: true, message: `Removed **${roleObj.name}** role from **${member.displayName}**.` };
    }
